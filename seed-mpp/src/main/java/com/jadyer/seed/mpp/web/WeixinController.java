...
public class WeixinController extends WeixinMsgControllerCustomServiceAdapter {
    ...
    protected WeixinOutMsg processInTextMsg(WeixinInTextMsg inTextMsg) {
        ...
        if("blog".equals(inTextMsg.getContent())){
            return new WeixinOutTextMsg(inTextMsg).setContent("[右哼哼]欢迎访问<a href=\"https://jadyer.cn/\">我的博客</a>[左哼哼]");
        }
        //回复多图文
        WeixinOutNewsMsg outMsg = new WeixinOutNewsMsg(inTextMsg);
        outMsg.addNews("第一个大图文标题", "第一个大图文描述", "http://avatar.csdn.net/6/0/B/1_jadyer.jpg", "https://jadyer.cn/");
        outMsg.addNews("第二个图文的标题", "第二个图文的描述", "http://img.my.csdn.net/uploads/201507/26/1437881866_3678.png", "https://github.com/jadyer");
        outMsg.addNews("第三个图文的标题", "第三个图文的描述", "http://img.my.csdn.net/uploads/201009/14/7892753_1284475095fyR0.jpg", "https://jadyer.cn/2015/10/20/wechat-dev-ngrok-pagekite/");
        return outMsg;
        */
        //防伪
    ...
    public WeixinErrorInfo createWeixinMenu(){
        ...
        WeixinSubViewButton btn11 = new WeixinSubViewButton("我的博客", "https://jadyer.cn/");
    ...
    public WeixinErrorInfo pushWeixinMsgToFans(){
        ...
        //推图文消息
        WeixinCustomNewsMsg.News.Article article11 = new WeixinCustomNewsMsg.News.Article("欢迎访问玄玉博客", "玄玉博客是一个开放态度的Java生态圈", "http://avatar.csdn.net/6/0/B/1_jadyer.jpg", "https://jadyer.cn/");
